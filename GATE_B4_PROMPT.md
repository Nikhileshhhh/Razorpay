...
2. **Idempotent re-execute wasn't idempotent.** Rebuilding-and-comparing the basis _before_ checking for an existing reservation meant a legitimate repeated/concurrent `execute` (same successful hash) could fail `APPROVAL_STALE` because the first call's own transitions had moved live state. **Fix:** in `reserveAction`, the existing-action lookup by (stable, content-derived) idempotency key happens **before** the freshness rebuild; a repeat returns the existing action, a genuinely new authorization still gets the full rebuild-and-compare.
